...
const summarizer = createAgent({
    ...
    model: openai({ model: 'gpt-4o', apiKey: process.env.OPENAI_AI_KEY })
})
...
export const meetingProcessing = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const transcriptWithSpeakers = await step.run("add-speakers", async () => {
            ...
            const agentSpeakers = await db.select().from(agents).where(inArray(agents.id, speakerIds)).then((agent) => ({
                ...agent
            }))
